...
joblib.dump(scaler, "StandardScaler_RF-01-v1.joblib")
...
# Run Random Forest instead of Lasso
...
joblib.dump(model_rf, "Stroke_RF-01-v1.joblib")

# Feature importance instead of coefficients
print("\n### PRIMARY RISK FACTORS ###")
top_risks = get_top_risk_factors(features, model_rf.feature_importances_, top_n=3)
print(top_risks)

# Save the top risk factors for the web app
os.makedirs(output_directory, exist_ok=True)
top_risks.to_csv(os.path.join(output_directory, "top_risk_factors.csv"), index=False, float_format="%.6f")

### CHECKING SCORES
RMSE_nocv = np.sqrt(mean_squared_error(y, model_rf.predict(X_train)))
AUC_nocv = roc_auc_score(y, model_rf.predict_proba(X_train)[:, 1])  # Use probability for AUC
print("\n Non cross-validated Random Forest RMSE: %.6f" % RMSE_nocv)
...
for i, (train_index, val_index) in enumerate(rkf_grid):
    ...
    scores_val = model_rf.predict_proba(Xval)[:, 1]  # Probability of positive class
    ...
    y_pred = model_rf.predict_proba(X_test)[:, 1]  # Probabilities for test set
...
test_predictions = model_rf.predict_proba(X_test)[:, 1]  # Get probability of stroke
...
print("\nTest prediction and prediction summary saved to 'prediction_summary.txt'.")
